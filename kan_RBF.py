...
class ImprovedRBFKAAGNN(torch.nn.Module):
    def __init__(self, N, dim, layer_hidden, layer_output, num_centers=5, 
        ...
        super(ImprovedRBFKAAGNN, self).__init__()
...
class MultiModalRBFKAAGNN(nn.Module):
    def __init__(self, N, dim, layer_hidden, layer_output, vocab_size,
        ...
        super(MultiModalRBFKAAGNN, self).__init__()
    ...
    def encode_smiles(self, smiles_list, input_ids_list, attention_mask_list):
        input_ids = torch.stack(input_ids_list)
        attention_mask = torch.stack(attention_mask_list)

        bert_outputs = self.molbert(input_ids, attention_mask)
    ...
    def forward_regressor(self, data_batch, train):
        ...
        if train:
            _, gnn_features, _ = self.gnn((Smiles, fingerprints, adjacencies, molecular_sizes))

            bert_features = self.encode_smiles(Smiles, input_ids_list, attention_mask_list)
...
class MultiModalRBFKAAGNNWithPretrainedBERT(nn.Module):
    def __init__(self, N, dim, layer_hidden, layer_output, 
                 ...
                 freeze_bert=False):
        super(MultiModalRBFKAAGNNWithPretrainedBERT, self).__init__()
    ...
    def encode_smiles_custom(self, input_ids_list, attention_mask_list):
        input_ids = torch.stack(input_ids_list)
        attention_mask = torch.stack(attention_mask_list)
    ...
    def forward_regressor(self, data_batch, train):
        ...
        if train:
            ...
            if self.use_pretrained:
                ...
                bert_features = self.encode_smiles_custom(input_ids_list, attention_mask_list)
